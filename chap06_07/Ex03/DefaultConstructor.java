...
// 생성자의 역할 : 1. 객체 생성, 2. 필드의 값을 초기화, 3. 
...
class B { 
	...
	// 객체 성성시 생성자를 통해서 필드의 초기값을 할당하고 객체 생성, 

class C { 
	int m;
	//  C () { }		// 기본 생성자는 생략됨 , 다른 생성자가 존재하는 경우 외부에서 기본 생성자 호출시
						// 기본 생성자가 생략되면 안된다.
	C (int c) { 		// 생성자의 인풋값을 받아서 메모리의 기본 값으로 m변수 값을 로드 시킨다.
		m = c; 
	}
...
public class DefaultConstructor {
	...
	public static void main(String[] args) {		// << 생성자의 특징 >> 
		// 클래스 내에 포함될 수 있는 것 : 1. 필드,  2. 메소드,  3. 생성자,  4.이너 클래스 (inner class) 
		...
		// 기본 생성자 (Default Constructor) : 생략 가능
			...
			C c = new C(3);		// << 생성자 호출시 기본 값을 인풋해서 메모리 필드에 초기값 구성 >> 
