...
class AA {		// 일반 클래스 : 객체 생성시 필드와 메소드는 Heap 메모리 영역에 저장
					// Heap : 반드시 초기화된 값을 가진다.
						// 필드는 Heap에 필드명과 값
						// 인스턴스 메소드는 : 클래스 영역의 인스턴스 영역에 메소드의 구현코드까지 저장
							//	Heap에는 포인터 정보만 가진다. 
	int a;
	double b;
	char c;
...
public class AbstractClass_1 {
	...
	public static void main(String[] args) {
		...
		System.out.println(aa);	// 객체를 생성 할려면 Heap 공간에 값이 반드시 들어가 있어야 한다.
		
		// 2. 추상메소드는 객체 생성 불가 : 메소드가 미완성 메소드이기 때문에 객체 생성이 불가.
