...
public class FileMethod {
	...
	public static void main(String[] args) {
		...
		System.out.println(newFile1.mkdir()); 		// 폴더를 잘 생성한 경우: true , 폴더가 존재해서 생성되지 못한 경우 false
			// 여러 이유로 인해서 false : 생성을 못했다. (해당 폴더가 존재하는 경우, 권한 문제로 만들 수 없는 경우, 하위폴더, ) 
		System.out.println(newFile1.exists());
		
		File newFile2 = new File("C:/temp/abc/bcd/cde"); 	// 하위의 폴더를 한꺼번에 생성하기.
		...
		//
		for (File k : fname) {
			System.out.println((k.isDirectory() ? "폴더 : " : "파일 : ") + k.getName());  // ( true면 폴더: , false면 파일 : )
		}
