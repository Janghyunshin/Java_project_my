...
// TreeSet의 이점:  1. 중복저장 되지 않는다.(Set)	2. 오름차순 정렬되어 입력.		3. 정렬되어 출력 (오름차순),	4. 검색기능을 사용 
				// 5. 특정범위의 값을 추출,	6. asc(Ascending : 오름차순 정렬), desc(Descending 내림차순 정렬)이 가능하다.
...
// TreeSet에 일반 객체를 저장할 경우 , 특정 필드를 크기비교 기준 필드가 존재해야 한다.
	//1. Comparable<E> 인터페이스의 compareTo() 메소드 재정의 : TreeSet에 일반 객체를 저장할때 일반 객체의 특정 필드를 재정의해야 한다.
		// 기존 클래스를 수정해서 사용	// comparable<E> 인터페이스를 상속 후 compareTo()메소드 재정의
		// 기존 클래스를 수정

	//2. Comparator<E> 인터페이스 compare() 메소드를 재정의 함 .	<<익명 내부 객체를 사용>
...
class MyComparableClass implements Comparable<MyComparableClass>{
	...
	public int compareTo(MyComparableClass o) {	// 객체의 크기비교 기준을 설정하는 메소드, <<TreeSet에서 사용>> (음수, 0, 양수)
		...
		if (this.data1 < o.data1) {			// this.data1 기준 값이 작을 경우 : -1 (음수) 		// 1  (내림차순으로 하고 싶을떄) 
			return -1 ;	
		} else if (this.data1 == o.data1) {	// this.data1 기준 값이 같을 경우 : 0 			// 0
			return 0 ;
		} else {							// this.data1 기준 값이 클 경우 : 1 (양수) 		// -1 (내림차순으로 하고 싶을떄)
			return 1;
		}
...
public class TreeSetMethod_2 {
	public static void main(String[] args) {
		...
		treeSet2.add(str2);		// Comparable<E> 인터페이스의 compareTo() 메소드가 재정의 되어 있어야 한다.
		
		System.out.println(treeSet2);
		
		// 3. MyClass가 TreeSet에 저장이 될까? , Comparable<E> 인터페이스의 compareTo() 메소드가 재정의 되어야 TreeSet 컬렉션에 저장가능 
...
//		treeSet3.add(myClass2);		// 실행 시 오류 발생됨.
//		
